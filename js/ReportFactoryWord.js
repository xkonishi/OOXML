/**
 * ReportFactory extension for Word
 */
if (!ReportFactory) {
...
(function() {
	 ...
	 * コンストラクタ
	 * 
	 * @param template_ : BASE64 string
	 * @param queryArray_ : JSON array
	 */
	ReportFactory.Word= function(template_, queryArray_) {
		this.pkg= new openXml.OpenXmlPackage(template_);
		this.queryArray= queryArray_;
	};
	
	/**
	 * データを差し込む。
	 * 
	 * @param data : sObject's as JSON array
	 */
	ReportFactory.Word.prototype.merge= function(data) {
		//メインドキュメント［word/document.xml］XML文書
		let mnXDoc = this.pkg.mainDocumentPart().getXDocument();

		//差し込みデータの挿入
		if (data.length > 0 && data[0].length > 0) {
			merge(data[0][0], mnXDoc);
		}
	};
	 ...
	 * レポートを出力する。
	 * 
	 * @param reportName : string レポート名
	 */
	ReportFactory.Word.prototype.save= function(reportName) {
